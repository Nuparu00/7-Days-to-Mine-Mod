...
public class CombustionGeneratorScreen extends AbstractContainerScreen<ContainerCombustionGenerator> {
	...
	public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		guiGraphics.drawString(minecraft.font,Component.translatable("gui.electricity.voltage",container.getProduction()),marginHorizontal+34,marginVertical+44,4210752);
		guiGraphics.drawString(minecraft.font,Component.translatable("gui.electricity.stored",container.getStored(),container.getCapacity()),marginHorizontal+34,marginVertical+54,4210752);
		this.renderTooltip(guiGraphics, mouseX, mouseY);
		if(ClientUtils.isInRect(marginHorizontal + 17,marginVertical + 19,142,9,mouseX,mouseY)){
			guiGraphics.renderTooltip(minecraft.font,Component.translatable("gui.overheating",(int)Math.round(container.getTemperature()*100)), mouseX, mouseY);
		}
	}
	...
	protected void renderBg(@NotNull GuiGraphics guiGraphics, float partialTicks, int x, int y) {
		...
		guiGraphics.blit(TEXTURE, this.getGuiLeft() + 8, this.getGuiTop() + 51 - yOffset, 176, 13 - yOffset, 14, yOffset);
		double temperature = container.getTemperature();
		guiGraphics.blit(TEXTURE, marginHorizontal + 19 + (int)(temperature*132), marginVertical+22, 190, 31, 6, 8);
	}
