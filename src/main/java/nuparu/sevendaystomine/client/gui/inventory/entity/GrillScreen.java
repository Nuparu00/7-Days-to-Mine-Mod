...
public class GrillScreen extends AbstractContainerScreen<ContainerGrill> {
	...
	protected void renderBg(PoseStack matrixStack, float partialTicks, int x, int y) {
		...
		this.blit(matrixStack, marginHorizontal + 88, marginVertical + 36, 176, 14, progressLevel + 1, 16);

		// draw the fuel remaining bar for each fuel slot flame
		double burnRemaining = container.getGrill().isBurning() ? 1 : 0;
		int yOffset = (int) (Math.ceil(burnRemaining * 14));
		this.blit(matrixStack, this.getGuiLeft() + 58, this.getGuiTop() + 75 - yOffset, 176, 12 - yOffset, 14, yOffset);
	}
