...
public class NotificationToast implements IToast
{
    ...
    public IToast.Visibility draw(GuiToast toastGui, long delta)
    {
        ...
        if (!stack.isEmpty())
        {
            List<String> list = toastGui.getMinecraft().fontRenderer.listFormattedStringToWidth(title.getFormattedText(), 125);
            ...
            else
            {
                ...
                if (delta < 1500L)
                {
                    ...
                    toastGui.getMinecraft().fontRenderer.drawString(I18n.format("unlocked.toast"), 30, 11, 16776960 | k);
                }
                else
                {
            ...
            if (!this.hasPlayedSound && delta > 0L)
            {
                ...
                if (true)
                {
                    toastGui.getMinecraft().getSoundHandler().playSound(PositionedSoundRecord.getRecord(SoundHelper.RECIPE_UNLOCKED, 1.0F, 1.0F));
                }
            }
