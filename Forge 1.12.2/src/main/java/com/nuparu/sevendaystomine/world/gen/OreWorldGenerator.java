...
public class OreWorldGenerator implements IWorldGenerator {
	...
	private final Predicate<IBlockState> stonePredicate = new Predicate<IBlockState>() {
        @Override public boolean apply(IBlockState state) {
	...
	private void generateOverworld(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator,
			IChunkProvider chunkProvider) {
		addOreSpawn(ModBlocks.ORE_COPPER.getDefaultState(),world,random,chunkX*16,chunkZ*16,16,16,12,32,10,138,stonePredicate);
		addOreSpawn(ModBlocks.ORE_TIN.getDefaultState(),world,random,chunkX*16,chunkZ*16,16,16,8,29,6,96,stonePredicate);
		addOreSpawn(ModBlocks.ORE_ZINC.getDefaultState(),world,random,chunkX*16,chunkZ*16,16,16,8,29,0,90,stonePredicate);
		addOreSpawn(ModBlocks.ORE_LEAD.getDefaultState(),world,random,chunkX*16,chunkZ*16,16,16,5,34,0,76,stonePredicate);
		addOreSpawn(ModBlocks.ORE_POTASSIUM.getDefaultState(),world,random,chunkX*16,chunkZ*16,16,16,8,20,32,128,stonePredicate);
		addOreSpawn(ModBlocks.ORE_CINNABAR.getDefaultState(),world,random,chunkX*16,chunkZ*16,16,16,4,16,24,96,stonePredicate);
