...
public class TransitProcess extends WindowedProcess {
		...
		if (this.isFocused()) {
			if (this.cursorCounter / 6 % 2 == 0) {
				if ((cursorPosition) <= text.length()) {
					...
					l = GuiUtilRenderComponents.splitText(new TextComponentString(ss),
							(int) Math.floor(width - 35), Screen.mc.fontRenderer, true, true);
					...
					// line counter
					renderCursor(left + 25 + Screen.mc.fontRenderer.getStringWidth(l.get(lineIndex).getUnformattedText()), top + lineIndex * Screen.mc.fontRenderer.FONT_HEIGHT, 1,
							Screen.mc.fontRenderer.FONT_HEIGHT, zLevel + 2);
				}
			}
		}
	...
	public void keyTyped(char typedChar, int keyCode) {
		...
		case 15: {
			...
			if (tree != null) {
				tree.print("-", true);
				CodeBlock block = Interpreter.read(tree,null);
			}
			break;
		}
		case 28:
